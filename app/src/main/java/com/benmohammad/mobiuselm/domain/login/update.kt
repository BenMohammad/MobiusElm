package com.benmohammad.mobiuselm.domain.login
        ...
        return when (event) {
            ...
            is LoginInputEvent -> {
                if(!validateLogin(event.login))
                    ...
                    Next.next(model.copy(pass = event.login, loginError = null, btnEnabled = validatePass(model.pass)))
            }
            is PassInputEvent -> {
            ...
            is IsSavedCredentialsEvent -> Next.next(model.copy(saveUser = event.checked))
